# -*- coding: utf-8 -*-
"""M21AIE225_DLOps_Assignment_3_Q_1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ly6qZnz7KiUIZubDXnZNcubOwZdGuOnB
"""
...
# Train the model
def model_training(model, criterion, optimizer, trainloader, testloader, num_epochs,trial):
    start = time.time()
    loss_list = []
...
def confusionMatrixAndAccuracyReport(Y_test, Y_pred, classes):
    ...
    plt.title(
        f'Accuracy : {overallAccuracy*100:3.2f}% | F1 Score : {f1_score*100:3.2f}% ', size=14)
    ...
    classwiseAccuracy_df = pd.DataFrame(data=[classwiseAccuracy], columns=classes)
...
with torch.no_grad():
    ...
    confusionMatrixAndAccuracyReport(test_labels, test_output, test_set.classes)
...
def objective(trial):
    ...
    act_fn_name = trial.suggest_categorical("activation", ["ReLU", "Tanh", "GELU"])
    ...
    model, criterion, optimizer, trainloader, testloader, num_epochs, trial)
    ...
    with torch.no_grad():
        ...
        test_labels = np.array(test_labels)
        test_output = np.array(test_output)
        metric = confusionMatrixAndAccuracyReport(test_labels, test_output, test_set.classes)
    return metric
